...
def activations(
    ...
    if area == 'v4':
        model = V4ColorTaskDriven(ensemble=True, centered=True)
        neurons = list(range(394))
        model_name = "V4ColorTaskDriven"
    else:
        model = V1GrayTaskDriven(ensemble=True, centered=True)
    ...
    sidx = idx.numpy()
    return sresps, sidx
    # endrespdir = os.path.join(data_dir, f"{area}_{dataset}_ordered_responses")
    # idxdir = os.path.join(data_dir, f"{area}_{dataset}_ordered_indices")
...
if __name__ == "__main__":
    ...
    activations(
        ...
        args.area, args.batch_size, args.num_workers
    )