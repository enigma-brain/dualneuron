...
It is saved to the non-staged ANALYSIS_DIR/{area}/{backbone}/mask.npy (the shipped
twins/<folder>/mask.npy files are read-only and never overwritten) and is the shared RF input to
screening and DreamSim, so each evaluates exactly the retinotopic region its neuron drives. For a
staged twin it reproduces the shipped mask (correlation > 0.99), reported as a QC comparison.
...
def compare_to_staged(mask, area, backbone):
    """
    Compare a computed mask to this twin's shipped (read-only) staged mask, if it has one.
    ...
    Returns:
        dict or None: {"corr", "mse", "max_abs"} versus the staged mask, or None if this twin
            has no staged mask (e.g. the DINO twins) or the shapes differ.
    """
    spec = registry.resolve(area, backbone)
    if spec.staged_folder is None:
        return None
    ref = os.path.join(TWINS_DIR, spec.staged_folder, "mask.npy")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--synthesis_dir", type=str, default=None,
                        help="synthesis npz folder (default ANALYSIS_DIR/{area}/{backbone}/synthesis)")
    parser.add_argument("--output", type=str, default=None,
                        help="output .npy (default ANALYSIS_DIR/{area}/{backbone}/mask.npy; "
                             "the read-only staged twins/ masks are never overwritten)")
    parser.add_argument("--log_path", type=str, default=None,
                        help="progress log file (default LOGS_DIR/{area}_{backbone}_mask.log)")
    ...
    if log_path is not None:
        ...
        log_file.write(
            f"build mask area={args.area} backbone={args.backbone} "
            f"percentile={args.percentile} sigma={args.sigma}\n")
        log_file.flush()
    ...
    cmp = compare_to_staged(mask, args.area, args.backbone)
    summary = (f"mask {mask.shape} from {n} neurons in {elapsed:.0f}s"
               + (f" | vs staged: corr={cmp['corr']:.4f} mse={cmp['mse']:.5f} "
